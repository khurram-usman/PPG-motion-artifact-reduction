...
for i = 1:numTotalWindows
    ...
    validInd = find( ((minFundamentalFreq/60) < f_IR) & (f_IR < (maxFundamentalFreq/60 + 0.25)) );
    ...
    fundamentalFreq_estHR = f_IR( validInd(maxInd_IR) );
    vec_fundamentalFreq_estHR(i) = fundamentalFreq_estHR;
    numHarmonics = floor( f_IR(1) / fundamentalFreq_estHR );
    harmonicInd = [];
    currMask = zeros( numel(f_IR) , 1 );
    for ind_Harmonics = 1 : numHarmonics

        currInd = find( ( max( (fundamentalFreq_estHR*ind_Harmonics - stdDev_HR*3) , f_IR(end) ) < f_IR) & (f_IR < min( (fundamentalFreq_estHR*ind_Harmonics + stdDev_HR*3) , f_IR(1) ) ) );
    ...
    xrec_G = icwt(cwt_G2  , [] , f_IR  , [f_IR(end) f_IR(1)] );
    curr_GData = icwt(cwt_IR  , [] , f_IR  , [f_IR(end) f_IR(1)] );
    if Ind_plotDebug
